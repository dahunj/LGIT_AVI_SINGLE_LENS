...
//	if(gData.bAgingMode) return FALSE;
//
...
void CLoopTimer::Takt_Save(int nZone, int nCase, const CString &sNote, int nValue)
{
	CString strLog, strZone, strMsg;
	
	g_objLogFile.Get_ZoneMsg(nZone, nCase, strZone, strMsg);	
	if(sNote != "") strMsg = sNote;

	strLog.Format("MCC,(%02d) %s,(%02d) %s,%0.3lf", nZone, strZone, nCase, strMsg, 0.000);
	g_objLogFile.Save_MCCLog(strLog);
