...
void CSequenceMain::Set_ClearRunData(BOOL bInit)
{
	...
	for(int i = 0; i < 7 ; i++)
	{
		...
		for(int j = 0; j < 10 ; j++) gLot.nGoodCountZig[i][j] = 0;
		for(int j = 0; j < 10 ; j++) gLot.nNgCountZig[i][j] = 0;
		for(int j = 0; j < 10 ; j++) gLot.nEmptyCountZig[i][j] = 0;

		gLot.dwStopTime[i] = 0; 
		gLot.dwErrorTime[i] = 0; 
...
void CSequenceMain::Job_LotEnd(int nMZNo)
{
	...
	for(int j = 0; j < 10 ; j++) gLot.nGoodCountZig[nMNo][j] = 0;
	for(int j = 0; j < 10 ; j++) gLot.nNgCountZig[nMNo][j] = 0;
	for(int j = 0; j < 10 ; j++) gLot.nEmptyCountZig[nMNo][j] = 0;
	

	gLot.dwStopTime[nMNo] = 0; 
	for(int j = 0; j < 10 ; j++) gLot.dwStopTimeZig[nMNo][j] = 0;

	gLot.dwErrorTime[nMNo] = 0;
	for(int j = 0; j < 10 ; j++) gLot.dwErrorTimeZig[nMNo][j] = 0;

	gLot.nErrorCount[nMNo] = 0;
	for(int j = 0; j < 10 ; j++) gLot.nErrorCountZig[nMNo][j] = 0;

	gLot.nStopCount[nMNo] = 0;
	for(int j = 0; j < 10 ; j++) gLot.nStopCountZig[nMNo][j] = 0;
	...
	gLot.sStartTime[nMNo] = "";
	gLot.sEndTime[nMNo] = "";

	for(int j = 0; j < 10 ; j++) gLot.sStartTimeZig[nMNo][j] = "";
	for(int j = 0; j < 10 ; j++) gLot.sEndTimeZig[nMNo][j] = "";

	for(int j = 0; j < 10 ; j++) gLot.dwStartTimeZig[nMNo][j] = 0;
	for(int j = 0; j < 10 ; j++) gLot.dwEndTimeZig[nMNo][j] = 0;
	for(int j = 0; j < 10 ; j++) gLot.dwInterval[nMNo][j] = 0;
...
BOOL CSequenceMain::LoadConveyorRun()
{
	...
	case 2:	
		if(g_objCommon.Get_LdStopper1Up())
		{
			...
			{
				m_nLoadConveyorLoop.Takt_Save(1, m_nLoadConveyorCase, "Load CV CW Run");
				g_objCommon.Set_LoadCVRunCW();
				m_nLoadConveyorCase++; m_nLoadConveyorLoop.Set_LoopTime(25000);
			}
			else if(m_pDX00->iLdCVMZExist1R)
			{
				m_nLoadConveyorLoop.Takt_Save(1, m_nLoadConveyorCase, "MGZ Already On Exist 1");
				m_nLoadConveyorCase++; m_nLoadConveyorLoop.Set_LoopTime(25000);
			}
		}		
		break;
	...
	case 5:
		if(!m_pDX00->iElvMZExist1 || !m_pDX00->iElvMZExist2)
		{
			...
			m_nLoadConveyorLoop.Takt_Save(1, m_nLoadConveyorCase, "Elv Start");
		...
		else if(m_pDX00->iElvMZExist1 && m_pDX00->iElvMZExist2)
		{
			...
			m_nLoadConveyorLoop.Takt_Save(1, m_nLoadConveyorCase, "Return to 0");
	...
	case 6:
		...
		{
			m_nLoadConveyorLoop.Takt_Save(1, m_nLoadConveyorCase, "go to Elv Wait");
...
BOOL CSequenceMain::MZElevRun()
{
	...
	case 2:
		...
		{	
			...
			if(nMZDetectCnt[0] > 5)
			{
				m_nMZElevLoop.Takt_Save(2, m_nMZElevCase, "Load MZ from Load CV to Elv(Load)");
				...
				g_objCommon.Set_LoadCVRunCW();theApp.uSleep(15);
				g_objCommon.Set_ElevCVRunCW();theApp.uSleep(15);
		...
		{				
			...
			if(nMZDetectCnt[1] > 5)
			{
				m_nMZElevLoop.Takt_Save(2, m_nMZElevCase, "Load MZ from Load CV to Elv(Rdy)");
				for(int i = 0; i < 6; i++) nMZDetectCnt[i] = 0;
				
				dwTick1 = GetTickCount();

				g_objCommon.Set_ElevLift1Down(); theApp.uSleep(10);
				g_objCommon.Set_ElevLift1Out(); theApp.uSleep(10);
				g_objCommon.Set_LoadCVRunCW();theApp.uSleep(5); //CW E¸AuCI·A¸e CCWμμ True ·I CØ¾ßCO 
		...
		{			
			...
			if(nMZDetectCnt[2] > 5)
			{
				m_nMZElevLoop.Takt_Save(2, m_nMZElevCase, "Slide Over");
				dwTick1 = GetTickCount();
		...
		{			
			...
			{
				dwTick1 = GetTickCount();
				m_nMZElevLoop.Takt_Save(2, m_nMZElevCase, "Elv Already Full");
				m_nLoadConveyorCase = eLoadCVBr::Check;
				for(int i = 0; i < 6; i++) nMZDetectCnt[i] = 0;
		...
		{	
			m_nMZElevLoop.Takt_Save(2, m_nMZElevCase, "Elv Else Case");
			nMZDetectCnt[5]++;
			if(nMZDetectCnt[5] > 5)
			{
				for(int i = 0; i < 6; i++) nMZDetectCnt[i] = 0;
	...
	case ElvBranch::LoadMZ:	
		...
		else
		{
			if(m_pDX00->iElvMZExist1)
			{
				...
				g_objCommon.Set_LoadCVStop(); theApp.uSleep(5);
				g_objCommon.Set_LoadCVRunCCW(); theApp.uSleep(5);
				g_objCommon.Set_ElevCVRunCW();
	...
	case 7:
		...
		{			
			...
			else
			{
				...
				m_nMZElevLoop.Takt_Save(2, m_nMZElevCase, "Job Lot Start - Manual");
	...
	case 8:
		...
		m_nMZElevLoop.Takt_Save(2, m_nMZElevCase, "Check_LotReady");
		gData.bElvLoadWait = FALSE;	
		m_nLoadConveyorCase = eLoadCVBr::Check;
	...
	case 82:
		if(GetTickCount() - dwTick1 < 3000)
		{
			...
			{				
				m_nMZElevLoop.Takt_Save(2, m_nMZElevCase, "Set_MGZIDReport");
				gMes.sMGZID[eMZ::Ready] = m_strBarcode[eBarcode::MZ-1]; m_strBarcode[eBarcode::MZ-1].Empty();
...
{	
	...
	case 2:
		...
		{
			m_strLog.Format("Elev Z pitch Move, SlotNo : %d", gData.nTNoPick);
	...
	case 3:
		...
		{						
			m_strLog.Format("FeederUnit X Move (MZ2), %d");
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 4:
		...
		{
			m_strLog.Format("Feeder Y Move (Zig Exist) ");
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 5:	
		...
		if(m_pDX01->iFeederZigExist)
		{
			...
			if(nMZDetectCnt[0] > 5)
			{
				...
				if(m_pEquipData->bUseMES)
				{
					m_strLog.Format("Feeder Y Move (Grip Zig)"); 
					m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 6:		
		...
		m_strLog.Format("Tray Number Add & Feeder Back to avoid : %d", gData.nTNoPick[eMZ::Load]);
		m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 7:
		if(g_objAJinAXL.Is_MoveDone(AX_ZIG_FEEDER_Y, dPosY))
		{
			if(gData.nTNoPick[eMZ::Load] > 10)
			{
				...
				m_strLog.Format("Tray Number Init : %d", gData.nTNoPick[eMZ::Load]);
				m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
				if(Check_TrayAllReturn())
				{
					m_nMZElevCase = ElvBranch::Unload;
			...
			dPosZ = m_pMoveData->dMZElevZ[eElv_Z::Down] + m_pEquipData->dElevPitchZ * (gData.nTNoPick[eMZ::Load] - 1);
			g_objAJinAXL.Move_Absolute(AX_MZ_ELEVATOR_Z, dPosZ);
			m_strLog.Format("Elev Z Pitch Move : %0.3lf", dPosZ);
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 8:
		...
		{
			m_strLog.Format("Feeder Move to Load Position");
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 9:		
		...
		{			
			m_strLog.Format("Feeder Move Done Check");
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 10:
		if(g_objCommon.Check_Position(AX_ZIG_FEEDER_Y, eFeeder_Y::MZLoad))
		{
			m_strLog.Format("Feeder Grip Close"); 
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
...
//#endif
	...
	case 15:
		...
		if(g_objCommon.Check_Position(AX_ZIG_FEEDER_Y, eFeeder_Y::Ready) )
		{
			...
			{
				m_strLog.Format("Set_RailAlignIn"); m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 72:
		if(g_objCommon.Check_Position(AX_ZIG_FEEDER_X, eFeeder_X::Barcode))
		{
			...
			m_strLog.Format("Tray Barcode Triggered"); 
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	case 79:		
		...
		m_strLog.Format("Barcode : %s", m_strBarcode[eBarcode::CtZig-1]); 
		m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);

		m_nFeederCase = 74;m_nFeederLoop.Set_LoopTime(10000);		
		break;
	case 73:
		if(GetTickCount() - dwTick < 5000)
		{			
			...
			if(gData.bSimulMode || gData.bAgingMode)
			{
				int nTemp = g_objCommon.Get_Random(1,99);
				if(nTemp < 25) m_strBarcode[eBarcode::CtZig-1].Format("CA1Test-%d", gData.nSlotNoFeeder);
				else if(nTemp < 50) m_strBarcode[eBarcode::CtZig-1].Format("CA2Test-%d", gData.nSlotNoFeeder);	
				else if(nTemp < 50) m_strBarcode[eBarcode::CtZig-1].Format("CA3Test-%d", gData.nSlotNoFeeder);	
				else m_strBarcode[eBarcode::CtZig-1].Format("CA4Test-%d", gData.nSlotNoFeeder);	
			}
	...
	case 74:
		if(m_pEquipData->bUseMES)
		{
			...
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Set_TrayIDReport");
			m_nFeederCase++; m_nFeederLoop.Set_LoopTime(5000);
			if(m_pEquipData->bUseMES) g_objMesAgent.Set_TrayIDReport(1, gData.sZigIDFeeder); 
	...
	case 75:
		...
		{
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Tray ID Confirmed");
		...
		{
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Feeder X Move to Tray-Grip Pos");
	...
	case 17:
		if(g_objCommon.Check_Position(AX_ZIG_FEEDER_X, eFeeder_X::TrayGrip) )
		{
			...
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Set_RailAlignOut");
	...
	case 34:
		if(g_objCommon.Get_FeederClose())
		{
			...
			gData.nTablePocketFeeder = gData.nTablePocketRail; gData.nTablePocketRail = 0;
	...
	case 38:
		if(g_objCommon.Get_FeederOpen() )
		{
			int nMNo = gData.nMZNoFeeder-1;
			int nZNo = gData.nSlotNoFeeder-1;


			int nLensCntZig = gLot.nGoodCountZig[nMNo][nZNo]+gLot.nEmptyCountZig[nMNo][nZNo]+gLot.nNgCountZig[nMNo][nZNo];
			...
			m_strLog.Format("Set_TrayCompleted"); m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);
	...
	/*	if(gData.bAgingMode || gData.bSimulMode)
		...
		else
		{
			...
			if(nMZDetectCnt[1] > 5)
			{
				m_nFeederLoop.Takt_Save(3, m_nFeederCase, "MZ detected");
	...
	case 56:
		...
		m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Feeder Y Back to avoid");
	...
	case 57:
		if(g_objAJinAXL.Is_MoveDone(AX_ZIG_FEEDER_Y, dPosY))
		{
			...
			{
				m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Tray No Init");
				...
				if(Check_TrayAllReturn())
				{
					m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Check_TrayAllReturn");
					m_nMZElevCase = ElvBranch::Unload;
				}
				m_nFeederCase = 0; m_nFeederLoop.Set_LoopTime(5000);
				break;
	...
	case 58:
		if(g_objAJinAXL.Is_MoveDone(AX_MZ_ELEVATOR_Z, dPosZ))
		{
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Feeder X & Y Move to Ready");
			g_objCommon.Move_Position(AX_ZIG_FEEDER_X, eFeeder_X::MZReady);
			g_objCommon.Move_Position(AX_ZIG_FEEDER_Y, eFeeder_Y::MZReady);
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "");
			m_nFeederCase++; m_nFeederLoop.Set_LoopTime(5000);
	...
	case 59:		
		...
		{			
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Feeder X&Y Move Done");
	...
	case 62:
		if(g_objCommon.Check_Position(AX_ZIG_FEEDER_Y, eFeeder_Y::TrayOpen))
		{
			...
			gData.nMZNoFeeder = gData.nMZNoMZRdy[gData.nTNoPick[eMZ::Ready] -1]; 
			gData.nMZNoMZRdy[gData.nTNoPick[eMZ::Ready] -1] = 0; 
	...
	case 81:
		if(g_objCommon.Get_RailAlignIn() && g_objCommon.Check_Position(AX_ZIG_FEEDER_Y, eFeeder_Y::Ready))
		{
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Feeder X Move to Barcode");
			g_objCommon.Move_Position(AX_ZIG_FEEDER_X, eFeeder_X::Barcode);			
			m_nFeederCase++; m_nFeederLoop.Set_LoopTime(5000);
		}
		break;
	case 82:
		if(g_objCommon.Check_Position(AX_ZIG_FEEDER_X, eFeeder_X::Barcode))
		{
			...
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Set_Trigger");
			m_nFeederCase++; m_nFeederLoop.Set_LoopTime(5000);
		}
		break;
	case 89:		
		...
		m_nFeederCase = 84; m_nFeederLoop.Set_LoopTime(10000);		
		break;
	case 83:
		if(GetTickCount() - dwTick < 5000)
		{			
	...
	case 84:
		if(m_pEquipData->bUseMES)
		{			
			m_strLog.Format("Set_TrayIDReport %s", gData.sZigIDFeeder);
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, m_strLog);

			gMes.bTrayIDConfirm = FALSE; //Load : 1
			m_nFeederCase++; m_nFeederLoop.Set_LoopTime(5000);
			g_objMesAgent.Set_TrayIDReport(1, gData.sZigIDFeeder); 
		}
		else m_nFeederCase++; m_nFeederLoop.Set_LoopTime(5000);
		break;
	case 85:
		if(m_pEquipData->bUseMES && gMes.bTrayIDConfirm)
		{
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "TrayIDConfirm");
		...
		{
			m_nFeederLoop.Takt_Save(3, m_nFeederCase, "Feeder X Move to TrayGrip");
...
BOOL CSequenceMain::ZigPickerRun()
{
	...
	case (int) eTrayPickerBr::Load:
		...
		break;
	case 2:
		if(g_objCommon.Check_Position(AX_ZIG_PICKER_Y, eZigPicker_Y::Load) && g_objCommon.Get_RailAlignOut())
		{
	...
	case 15:
		...
		if(g_objCommon.Get_TrayPickMasterSlaveOut() )
		{
			...
			gData.nTablePocketMainIndex[eMainIndex::Load] = g_objCommon.Get_TablePocketNo();
	...
	case 25:
		if(g_objCommon.Get_TrayPickMasterSlaveIn())
		{
			...
			gData.nTablePocketTrayPick = gData.nTablePocketMainIndex[eMainIndex::Unload]; gData.nTablePocketMainIndex[eMainIndex::Unload] = 0;
	...
	case 30:
		if(g_objCommon.Get_TrayPickMasterSlaveOut() )
		{
			...
			gData.nTablePocketRail = gData.nTablePocketTrayPick; gData.nTablePocketTrayPick = 0;
...
BOOL CSequenceMain::LensCleanerRun()
{
	...
	case 6:
		...
		{			
			m_strLog.Format("Lens Cleanner Backward Done"); m_nLensCleanerLoop.Takt_Save(5, m_nLensCleanerCase, m_strLog);
			m_nLensCleanerCase++; m_nLensCleanerLoop.Set_LoopTime(gData.nLTime[eLT::Motion]);				
		}
		break;
	case 7:
...
BOOL CSequenceMain::TopInspectorRun()
{
	...
	case 1:
		...
		{
			m_strLog.Format("AxisNo,%d, Current,%0.3lf", AX_TOP_INSPECTOR_Y, g_objAJinAXL.Get_Position(AX_TOP_INSPECTOR_Y));	g_objLogFile.Save_PositionLog(m_strLog);
	...
	case 31:
		...
		if(m_sRecipe != "")
		{
			...
			m_strLog.Format("Period:%0.2lf,PitchX:%0.2lf,PitchY:%0.2lf,dStartX:%0.2lf,dStartY:%0.2lf,dStartZ:%0.2lf,dVel:%0.2lf,Count:%d",
				gRcp.dPeriod[eVision::TC], gRcp.dPitchX[eVision::TC], gRcp.dPitchY[eVision::TC], gRcp.dStartX[eVision::TC],gRcp.dStartY[eVision::TC],gRcp.dStartZ[eVision::TC], gRcp.dVelocity[eVision::TC], gRcp.nCount[eVision::TC]);
			g_objLogFile.Save_TestLog(m_strLog);
	...
	case 3:
		if(Select_TopScanPos(nTopXPos, nTopYPos, m_pEquipData->nVisionDir))
		{
			...
			dTopUnitY = gRcp.dStartY[eVision::TC] - (gRcp.dPitchY[eVision::TC] * (nTopYPos - 1)) + gData.dDeltaY[eMainIndex::Top];
			dTopUnitX = gRcp.dStartX[eVision::TC] + (gRcp.dPitchX[eVision::TC] * (nTopXPos - 1)) + gData.dDeltaX[eMainIndex::Top];
		...
		break;
	case 4:
		if (g_objAJinAXL.Is_Done(AX_TOP_INSPECTOR_Y) &&
			g_objAJinAXL.Is_Done(AX_TOP_INSPECTOR_X) &&
			g_objAJinAXL.Is_Done(AX_TOP_INSPECTOR_Z))
		{
			m_strLog.Format("AxisNo,%d, Current,%0.3lf", AX_TOP_INSPECTOR_Y, g_objAJinAXL.Get_Position(AX_TOP_INSPECTOR_Y));	g_objLogFile.Save_PositionLog(m_strLog);
			...
			{							
				...
				, gData.sZigIDMainIndex[eMainIndex::Top], gData.nSlotNoMainIndex[eMainIndex::Top], g_objCommon.ConvertToMESNo(nLensNo));				
	...
	case 7:		// Scan Move
		if (g_objAJinAXL.Is_Done(AX_TOP_INSPECTOR_Z)) 
		{
			double dPeriod = gRcp.dPeriod[eVision::TC];//m_pEquipData->dTopPeriod;	
			double dTrigS =  gRcp.dStartZ[eVision::TC];//m_pEquipData->dTopStartZ;				
			double dTrigE = dTrigS + (dPeriod) * (gRcp.nCount[eVision::TC]-1);	// Trigger End
			dTopZ = dTrigE + (dPeriod/10);								// Motion End (가감속)
			double dVelocity = gRcp.dVelocity[eVision::TC];
...
BOOL CSequenceMain::BtmInspectorRun()
{
	...
	case 1:
		...
		//Auto Recipe Change 
	case 31:
		m_sRecipe = gData.sZigIDMainIndex[eMainIndex::Btm].Left(3);
		if(m_sRecipe != "")
		{
			if(m_sRecipe == "CA1")
			{
			...
			g_objLogFile.Save_TestLog(m_strLog);
			m_strLog.Format("Period:%0.2lf,PitchX:%0.2lf,PitchY:%0.2lf,dStartX:%0.2lf,dStartY:%0.2lf,dStartZ:%0.2lf,dVel:%0.2lf,Count:%d",
				gRcp.dPeriod[eVision::BC], gRcp.dPitchX[eVision::BC], gRcp.dPitchY[eVision::BC], gRcp.dStartX[eVision::BC], gRcp.dStartY[eVision::BC], gRcp.dStartZ[eVision::BC], gRcp.dVelocity[eVision::BC], gRcp.nCount[eVision::BC]);
			g_objLogFile.Save_TestLog(m_strLog);
			m_nBtmInspectCase = 32; m_nBtmInspectLoop.Set_LoopTime(10000);	
		}
		break;
	case 32:
		if(gData.bRcpChange[eVision::BC])
		{
			m_nBtmInspectCase = 2; m_nBtmInspectLoop.Set_LoopTime(10000);	
		}
		break;
	...
	case 3:
		...
		{						
			...
			dBtmUnitY = gRcp.dStartY[eVision::BC] - (gRcp.dPitchY[eVision::BC] * (nBtmYPos - 1)) + gData.dDeltaY[eMainIndex::Btm];
			dBtmUnitX = gRcp.dStartX[eVision::BC] + (gRcp.dPitchX[eVision::BC] * (nBtmXPos - 1)) + gData.dDeltaX[eMainIndex::Btm];
		...
		{		
			...
			else 
			{
				...
				g_objInspector.Set_LoadComplete("BC", gData.sMZIDMainIdex[eMainIndex::Btm],gData.nMZNoMainIndex[eMainIndex::Btm],
					gData.sZigIDMainIndex[eMainIndex::Btm], gData.nSlotNoMainIndex[eMainIndex::Btm],g_objCommon.ConvertToMESNo(nLensNo));				
	...
	case 7:		// Scan Move
		if (g_objAJinAXL.Is_Done(AX_BTM_INSPECTOR_Z)) 
		{
			double dPeriod = gRcp.dPeriod[eVision::BC]; 	// 33mm
			double dTrigS = gRcp.dStartZ[eVision::BC];		// Trigger Start
			double dTrigE = dTrigS + dPeriod * (gRcp.nCount[eVision::BC] - 1) ;	// Trigger End
			dBtmZ = dTrigE + (dPeriod/10);								// Motion End (가감속)
			double dVelocity = gRcp.dVelocity[eVision::BC];
...
{	
	...
	case 31:
		...
		if(!gData.bIndexDone[eMainIndex::Top2] && !Check_IndexEmpty(eMainIndex::Top2) &&
			...
			g_objAJinAXL.Is_Done(AX_MARK_UNIT_Y) &&
		...
		//Auto Recipe Change 
	case 61:
		m_sRecipe = gData.sZigIDMainIndex[eMainIndex::Top2].Left(3);
		if(m_sRecipe != "")
		{
			if(m_sRecipe == "CA1")
			{
	...
	case 32:
		...
		{			
			...
			m_strLog.Format("Top Vision Use"); m_nTopInspectLoop.Takt_Save(6, m_nTopInspectCase, m_strLog);	
			Init_Top2Zig();
			nTop2XPos = 1; nTop2YPos = 1;	
			gData.dDeltaY[eMainIndex::Top2] = 0; gData.dDeltaX[eMainIndex::Top2] = 0;
			m_nMarkUnitCase++; m_nMarkUnitLoop.Set_LoopTime(10000);													
		}
		break;
	case 33:
		if(Select_Top2ScanPos(nTop2XPos, nTop2YPos, m_pEquipData->nVisionDir))
		{
			...
			{
				nLensNo = (gData.nLensCntY * (nTop2XPos-1)) + nTop2YPos;
			}
		...
		{
			//Done
			m_nMarkUnitCase = 45; m_nMarkUnitLoop.Set_LoopTime(10000);
		}
		break;
	case 34:
			...
			g_objAJinAXL.Is_Done(AX_MARK_UNIT_Z))
		{
			...
			if(gData.bAgingMode) if(!m_nMarkUnitLoop.Waiting_Time(250)) break;

			if (!m_pEquipData->bUseTop2Vision)
			{
				if (gData.InfoMainIndex[eMainIndex::Top2][nTop2XPos-1][nTop2YPos-1] == eLensState::Top2Ready)
					gData.InfoMainIndex[eMainIndex::Top2][nTop2XPos-1][nTop2YPos-1] = eLensState::Top2Done;	//Scan Done

				m_nMarkUnitCase = 40; m_nMarkUnitLoop.Set_LoopTime(30000);
			} 
			else 
			{							
	...
	case 40:		
		...
		{
			nTop2XPos++;
			if(nTop2XPos > gData.nLensCntX) 
			{
				nTop2YPos++; nTop2XPos = 1;
			}
		}
		else if(m_pEquipData->nVisionDir == eVDir::fixX)
		{
			nTop2YPos++;
			if(nTop2YPos > gData.nLensCntY) 
			{
	...
	case 45:
		{
			m_strLog.Format("Top Vision 2 Done"); m_nMarkUnitLoop.Takt_Save(8, m_nMarkUnitCase, m_strLog);
	...
	case 2:
		if(gData.bSimulMode || gData.bAgingMode)
		{
			m_pDX02->IMarkPenExist = TRUE;
			m_pDX02->IMarkPenDown = TRUE;
			m_pDX02->IMarkPenUp = FALSE;
		}
	...
	case 3:
		...
		if(!m_qLensNoNG.empty())
		{
			nLensNo = m_qLensNoNG.front();
			m_qLensNoNG.pop();
			m_strLog.Format("Lens No popped , %d", nLensNo); g_objLogFile.Save_HandlerLog(m_strLog);
						
			if(m_pEquipData->nVisionDir == eVDir::fixY)
			{
				nMarkYPos = ((nLensNo-1) / gData.nLensCntX);
				nMarkXPos = (nLensNo-1) % gData.nLensCntX;
			}
			else
			{
				nMarkXPos = ((nLensNo-1) / gData.nLensCntY);
				nMarkYPos = (nLensNo-1) % gData.nLensCntY;
			}	
			...
			dMarkUnitY = m_pMoveData->dMarkUnitY[eMark_Y::MarkStart] - (m_pEquipData->dZigPitchY * (nMarkYPos )) + gData.dDeltaY[eMainIndex::Mark];
			dMarkUnitX = m_pMoveData->dMarkUnitX[eMark_X::MarkStart] + (m_pEquipData->dZigPitchX * (nMarkXPos )) + gData.dDeltaX[eMainIndex::Mark];
	...
	case 6:
		...
		if(gData.nInspectInfo[nMNo][nTNo][nLensNo-1] == eLensInfo::NG)
		{
			...
			gData.InfoMainIndex[eMainIndex::Mark][nMarkXPos][nMarkYPos] = eLensState::Marked;	//Scan Done
			m_nMarkUnitCase++; m_nMarkUnitLoop.Set_LoopTime(10000);
		}			
		else
		{
			gData.InfoMainIndex[eMainIndex::Mark][nMarkXPos][nMarkYPos] = eLensState::NotMarked;	//Scan Done
			m_nMarkUnitCase = 10; m_nMarkUnitLoop.Set_LoopTime(gData.nLTime[eLT::Motion]);
		}
		break;
	case 7:
	...
	case 8:
		...
		{
			m_pDY02->oMarkPenDown = FALSE;m_pDY02->oMarkPenUp = TRUE;
			g_objAJinAXL.Write_Output(2);
			g_objCommon.Move_Position(AX_MARK_UNIT_Z, eMark_Z::MarkStart);
			m_nMarkUnitCase = 9; m_nMarkUnitLoop.Set_LoopTime(10000);
		}		
		break;	
	case 9:
		if(g_objCommon.Check_Position(AX_MARK_UNIT_Z, eMark_Z::MarkStart) && m_pDX02->IMarkPenExist 
			&& !m_pDX02->IMarkPenDown && m_pDX02->IMarkPenUp)
		{			
			m_nMarkUnitCase = 10; m_nMarkUnitLoop.Set_LoopTime(10000);
		}		
...
{	
	...
	case 4:
		...
		if(!gData.bIndexDone[eMainIndex::Mark]) m_nMarkUnitCase = 31;
	...
	case 10:
		if(Check_IndexDone()) // && g_objCommon.Get_IndexLoadAlignIn()
		{
				...
				&& g_objCommon.Check_Position(AX_MARK_UNIT_Z, eMark_Z::Ready))				 
			{
				...
				m_nIndexTLoop.Takt_Save(9, m_nMainIndexCase, "Check_IndexDone");
	...
	case 11:
		...
		if(g_objCommon.Get_IndexLoadAlignIn() && (m_pDX02->iIndexTZigExist || Check_IndexEmpty(0)))
		{
			...
			m_nIndexTLoop.Takt_Save(9, m_nMainIndexCase, "Rotation 60 degree");
			m_nMainIndexCase++; m_nIndexTLoop.Set_LoopTime(10000);
		}
		break;
	case 12:
		...
		{			
			Set_IndexEnd();
			m_nIndexTLoop.Takt_Save(9, m_nMainIndexCase, "Rotation Done");
...
BOOL CSequenceMain::UnloadConveyorRun()
{
	...
	case 2:
		if(nMZCntPre < 4)
		{
			dwTick = GetTickCount();
			g_objCommon.Set_UnloadCVRunCW();
			m_nIndexTLoop.Takt_Save(10, m_nUnloadConveyorCase, "");
			m_nUnloadConveyorCase++; m_nUnloadConveyorLoop.Set_LoopTime(gData.nLTime[eLT::CV]);
		}	
		else
		{
	...
	case 3:
		if(m_pDX01->iUldCvMZExist1L)
		{
			...
			if(nDetectCnt[0] > 5)
			{
				m_nUnloadConveyorLoop.Takt_Save(10, m_nUnloadConveyorCase, "iUldCvMZExist1L");
				nMZCntPost++; nDetectCnt[0] = 0;
			}
		}
		if(m_pDX01->iUldCvMZExist2)
		{
			nDetectCnt[1]++;
			if(nDetectCnt[1] > 5){ 
				m_nUnloadConveyorLoop.Takt_Save(10, m_nUnloadConveyorCase, "iUldCvMZExist2");
				nMZCntPost++; nDetectCnt[1] = 0;
			}
		}
		if(m_pDX01->iUldCvMZExist3)
		{
			nDetectCnt[2]++;
			if(nDetectCnt[2] > 5){ 
				m_nUnloadConveyorLoop.Takt_Save(10, m_nUnloadConveyorCase, "iUldCvMZExist3");
				nMZCntPost++; nDetectCnt[2] = 0; }
		}
		if(m_pDX01->iUldCvMZExist4)
		{
			nDetectCnt[3]++;
			if(nDetectCnt[3] > 5){
				m_nUnloadConveyorLoop.Takt_Save(10, m_nUnloadConveyorCase, "iUldCvMZExist4");
				nMZCntPost++; nDetectCnt[3] = 0; }
		}
		nDetectCnt[5]++;
		if(nDetectCnt[5] > 8)
		{
			for(int i = 0; i < 6; i++) nDetectCnt[i] = 0;
			m_nUnloadConveyorLoop.Takt_Save(10, m_nUnloadConveyorCase, "Detection End");
			m_nUnloadConveyorCase++;m_nUnloadConveyorLoop.Set_LoopTime(gData.nLTime[eLT::CV]);
		}				
		break;
	case 4:
		...
		{
			g_objCommon.Set_UnloadCVStop();
			m_nUnloadConveyorLoop.Takt_Save(10, m_nUnloadConveyorCase, "Set_UnloadCVStop");
			m_nUnloadConveyorCase++; m_nUnloadConveyorLoop.Set_LoopTime(gData.nLTime[eLT::CV]);			
		}
	...
	case 5:
		...
		m_nUnloadConveyorLoop.Takt_Save(10, m_nUnloadConveyorCase, "Set_LdStopper1Down");
		m_nUnloadConveyorCase++; m_nUnloadConveyorLoop.Set_LoopTime(gData.nLTime[eLT::CV]);	
	case 6:
		if(gData.bAgingMode || gData.bSimulMode)
		{
		...
		dwTick = GetTickCount();
		m_nUnloadConveyorLoop.Takt_Save(10, m_nUnloadConveyorCase, "Unload C/V Init");
		m_nUnloadConveyorCase = 0; m_nUnloadConveyorLoop.Set_LoopTime(gData.nLTime[eLT::CV]);
		break;	
...
void CSequenceMain::Begin_MainRunThread()
{
	...
	{	
		...
		for(int i = 0; i < 3; i++)
		{
			...
			for(int j = 0; j < 10; j++) gLot.dwErrorTimeZig[i][j] += gLot.dwErrorEnd;
		}
		gLot.dwErrorStart = 0;
		gLot.dwErrorEnd = 0;
	}
...
void CSequenceMain::Set_IndexEnd()
{
	...
	gData.nTablePocketMainIndex[eMainIndex::Unload] = gData.nTablePocketMainIndex[eMainIndex::Mark];
	gData.nTablePocketMainIndex[eMainIndex::Mark] = gData.nTablePocketMainIndex[eMainIndex::Btm];
	gData.nTablePocketMainIndex[eMainIndex::Btm] = gData.nTablePocketMainIndex[eMainIndex::None];
	gData.nTablePocketMainIndex[eMainIndex::None] = gData.nTablePocketMainIndex[eMainIndex::Top];
	gData.nTablePocketMainIndex[eMainIndex::Top] = gData.nTablePocketMainIndex[eMainIndex::Clean];
...
void CSequenceMain::Init_Top2Zig()
{
	for(int i = 0; i < gData.nLensCntX; i++)
	{
		for(int j = 0; j < gData.nLensCntY; j++)
		{
			if(gData.InfoMainIndex[eMainIndex::Top2][i][j] > 0)
			{
				gData.InfoMainIndex[eMainIndex::Top2][i][j] = eLensState::Top2Ready;
			}
		}
	}	
}
...
BOOL CSequenceMain::Select_Top2ScanPos(int &nTop2PosX, int &nTop2PosY, int nDir)
{
	// Module¸¸ Scan
	nTop2PosX = nTop2PosY = 0;
	...
	if(nDir == eVDir::fixX)
	{
		for(int i= 0; i < gData.nLensCntX; i++) 
		{
			if (i==1 || i==3 || i==5 || i==7 || i==9 || i==11 || i==13 || i==15 || i==17)
			{
				for(int j = gData.nLensCntY - 1; j >= 0; j--) 
				{
					if (gData.InfoMainIndex[eMainIndex::Top2][i][j] == eLensState::Top2Ready)  
					{					
						nTop2PosY = j + 1;
						nTop2PosX = i + 1;					
						break;
			...
			if (nTop2PosY > 0) break;
		}

		if (nTop2PosX > gData.nLensCntX) return FALSE;
		if (nTop2PosY == 0 || nTop2PosX == 0) return FALSE;
	}
	else if(nDir == eVDir::fixY)
	{
		for(int i= 0; i < gData.nLensCntY; i++) 
		{
			if (i==1 || i==3 || i==5 || i==7 || i==9 || i==11 || i==13 || i==15 || i==17)
			{
				for(int j = gData.nLensCntX - 1; j >= 0; j--) 
				{
					if (gData.InfoMainIndex[eMainIndex::Top2][j][i] == eLensState::Top2Ready)  
					{					
						nTop2PosY = i + 1;
						nTop2PosX = j + 1;					
						break;
			...
			if (nTop2PosX > 0) break;
		}

		if (nTop2PosY > gData.nLensCntY) return FALSE;
		if (nTop2PosY == 0 || nTop2PosX == 0) return FALSE;
	}	
	return TRUE;
}
...
BOOL CSequenceMain::Check_InspectDone(const CString& sZigID, int nMZNo, int nTNo, int nLNo, int& nInfo)
{
	int nMNo = nMZNo - 1;
	int nSlot = nTNo - 1;
	int nLens = nLNo - 1;
	...
	if (!bDone) 
	{
		if (m_pEquipData->bUseInspectSkip )//if (m_pEquipData->bUseInspectSkip || ((dwTick - gData.dwInspectWait)  > 5500)) 
		{
...
void CSequenceMain::Write_LotJudge(int nMZNo, int nTrayNo, int nLensNo, int nInfo)
{
	...
	int nFlag = 0;
	if(strResult =="Pass") nFlag = 1;
	else				   nFlag = 0;

	int nMarking = 0;
	if(m_pEquipData->bUseMark) nMarking = 1;
	else nMarking = 0;
...
void CSequenceMain::Get_RecipeData(int nVision, CString sRcp)
{
	...
	{
		gRcp.dStartZ[nVision]	= INI.Get_Double("TRIGGER",  "TOP_START_Z", 0.00);
		gRcp.nCount[nVision]	= INI.Get_Integer("TRIGGER", "TOP_COUNT",	 0);
		gRcp.dPeriod[nVision]	= INI.Get_Double("TRIGGER",  "TOP_PERIOD",  0.00);
		gRcp.dVelocity[nVision]	= INI.Get_Double("TRIGGER",  "TOP_VEL",	 0.00);
		gRcp.dStartX[nVision]	= INI.Get_Double("TRIGGER",  "TOP_START_X", 0.00);
		gRcp.dStartY[nVision]	= INI.Get_Double("TRIGGER",  "TOP_START_Y", 0.00);

		gRcp.dPitchX[nVision]	= INI.Get_Double("COAT_ZIG",  "PITCH_X", 0.00);
		gRcp.dPitchY[nVision]	= INI.Get_Double("COAT_ZIG",  "PITCH_Y", 0.00);
	}
	else if(nVision == eVision::BC)
	{
		gRcp.dStartZ[nVision]	= INI.Get_Double("TRIGGER",  "BTM_START_Z", 0.00);
		gRcp.nCount[nVision]	= INI.Get_Integer("TRIGGER", "BTM_COUNT",	 0);
		gRcp.dPeriod[nVision]	= INI.Get_Double("TRIGGER",  "BTM_PERIOD",  0.00);
		gRcp.dVelocity[nVision]	= INI.Get_Double("TRIGGER",  "BTM_VEL",	 0.00);
		gRcp.dStartX[nVision]	= INI.Get_Double("TRIGGER",  "BTM_START_X", 0.00);
		gRcp.dStartY[nVision]	= INI.Get_Double("TRIGGER",  "BTM_START_Y", 0.00);
	...
	else if(nVision == eVision::TC2)
	{
		...
		gRcp.dPeriod[nVision]	= INI.Get_Double("TRIGGER",  "TOP2_PERIOD",  0.00);
		gRcp.dVelocity[nVision]	= INI.Get_Double("TRIGGER",  "TOP2_VEL",	 0.00);
		gRcp.dStartX[nVision]	= INI.Get_Double("TRIGGER",  "TOP2_START_X", 0.00);
		gRcp.dStartY[nVision]	= INI.Get_Double("TRIGGER",  "TOP2_START_Y", 0.00);
...
BOOL CSequenceMain::isDoneLens(int nNo) 
{
	// 원본 큐를 보호하기 위해 복사본 사용
	std::queue<int> tempQueue = m_qLensNoDone;

	while (!tempQueue.empty())
	{
		if (tempQueue.front() == nNo)
		{
			return true;
		}
		tempQueue.pop();
