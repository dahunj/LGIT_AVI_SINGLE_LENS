...
BOOL CSequenceMain::FeederRun()
{
	...
	case (int) FeederBranch::LoadSearch:		
		...
		{
			//아래 부터 검사 
			dPosZ = m_pMoveData->dMZElevZ[eElev_Z::Down] - m_pEquipData->dElevPitchZ * (gData.nSlotNoToPick - 1) ;
	...
	case 3:
		...
		{
			//g_objCommon.Move_Position(AX_ZIG_FEEDER_Y, eFeeder_Y::CheckExist);
			m_nFeederCase++; m_nFeederLoop.Set_LoopTime(5000);
...
//#endif
	...
	case 35:
		...
		break;
	case 36:
		if(g_objCommon.Check_Position(AX_ZIG_FEEDER_Y, eFeeder_Y::MZLoad) && m_pDX01->iFeederZigExist)
		{
...
BOOL CSequenceMain::ZigPickerRun()
{
	...
	case (int) ZigPickBranch::Load:
		if(g_objCommon.Check_Position(AX_ZIG_PICKER_Z, eZigPicker_Z::Ready) && !m_pDX01->iZigPickerExist)
		{
	...
	case 10:
		...
		return TRUE;
	case 11:
		if(g_objCommon.Check_Position(AX_ZIG_PICKER_Z, eZigPicker_Z::Ready) && m_pDX01->iRailZigExist)
		{				
...
BOOL CSequenceMain::BtmInspectorRun()
{
	...
	case 3:
		if(Select_TopScanPos(nBtmXPos, nBtmYPos))
		{
			...
			dBtmUnitY = m_pMoveData->dTopInspectorY[eTopInspect_Y::ScanStart] + (m_pEquipData->dZigPitchY * (nBtmYPos - 1));
			dBtmUnitX = m_pMoveData->dTopInspectorX[eTopInspect_X::ScanStart] + (m_pEquipData->dZigPitchX * (nBtmXPos - 1));
			dBtmUnitZ = m_pMoveData->dTopInspectorZ[eTopInspect_Z::ScanStart];
...
{	
	...
	case 10:
		...
		if(Check_IndexDone()) // && g_objCommon.Get_IndexLoadAlignIn()
		{
			...
			if ((g_objAJinAXL.Get_Position(AX_ZIG_PICKER_Y) <= 200.0 
				&& g_objCommon.Check_Position(AX_ZIG_PICKER_Z, eZigPicker_Z::Ready)) 
...
BOOL CSequenceMain::Run_Simulation()
{
	...
	if(m_nMZElevCase == 3)
	{
		m_pDX00->iElvMZExist1 = TRUE;
	}

	if(m_nMZElevCase == 4)
	{
		m_pDX00->iElvMZExist2 = TRUE;
		g_objCommon.Set_ElevStopper2Down();
	}
